...
if (Notification.permission === "granted") { const title = "🌅 Günün Manevi İkramı Hazır"; const options = { body: 💡 Hikmet: "${dailyWisdom.quote.substring(0, 50)}..."\n🎯 Görev: ${dailyTask.text}, icon: "/favicon.ico", badge: "/favicon.ico", vibrate: [200, 100, 200], tag: "daily-wisdom", requireInteraction: true };

const notification = new Notification(title, options);

notification.onclick = (event) => { event.preventDefault(); window.focus(); notification.close();
...
// ZİNCİR MANTIĞI const lastVisit = localStorage.getItem('lastVisit'); let currentStreak = parseInt(localStorage.getItem('streak') || '0');

if (lastVisit !== today) { const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);

if (lastVisit === yesterday.toDateString()) { currentStreak += 1; } else { currentStreak = 1; } localStorage.setItem('lastVisit', today); localStorage.setItem('streak', currentStreak.toString()); } setStreak(currentStreak);

// BİLDİRİM PENCERESİ KONTROLÜ const hasAnswered = localStorage.getItem('notificationAsked');

if (!hasAnswered && 'Notification' in window && Notification.permission === 'default') { const timer = setTimeout(() => { setShowNotificationModal(true); }, 3000); return () => clearTimeout(timer); }

// GÜNLÜK OTOMATİK BİLDİRİM const lastNotificationDate = localStorage.getItem('lastNotificationDate'); if (Notification.permission === "granted" && lastNotificationDate !== today) { sendMorningNotification(); localStorage.setItem('lastNotificationDate', today); }
...
Notification.requestPermission().then((permission) => { if (permission === "granted") { setTimeout(() => { alert("Teşekkürler! Sabah virdiniz her gün cihazınıza iletilecektir."); }, 300); sendMorningNotification(); localStorage.setItem('lastNotificationDate', new Date().toDateString()); } });
...
{/* --- PAYLAŞIM MODALI --- */} {showSharePreview && ( <SharePreviewModal dailyWisdom={dailyWisdom} onClose={() => setShowSharePreview(false)} /> )}

{/* --- BİLDİRİM İZİN MODALI --- */} {showNotificationModal && (
...
// --- ORTAK KART BİLEŞENİ --- function StoryCardContent({ dailyWisdom }) { return ( <div className="w-[1080px] h-[1920px] bg-[#0F4C5C] flex flex-col items-center justify-between text-center relative overflow-hidden"> {/* Arka Plan Deseni */} <div className="absolute inset-0 bg-gradient-to-b from-[#0F4C5C] via-[#09303a] to-[#04151a]"></div> <div className="absolute inset-0 opacity-10 pointer-events-none" style={{backgroundImage: "url('https://www.google.com/search?q=https://www.transparenttextures.com/patterns/cubes.png')"}}> </div>
    ...
    <div className="z-10 mb-32 w-full px-12 flex flex-col items-center gap-10">
        <div className="bg-white p-6 rounded-[3rem] shadow-[0_0_50px_rgba(229,193,124,0.3)] border-[10px] border-[#E5C17C]">
            {/* QR KODU: Yeni Adres (https://www.onikikapi.com/) */}
        ...
        <div className="bg-[#09303a] px-16 py-6 rounded-full border-2 border-[#E5C17C]/50 shadow-lg">
            <p className="text-4xl text-[#E5C17C] tracking-wider font-bold font-mono">www.onikikapi.com</p>
        </div>
        
        <div className="flex flex-col items-center mt-4">
            <h1 className="text-[12rem] font-black text-[#E5C17C] leading-[0.8] tracking-tighter font-sans drop-shadow-2xl" style={{ textShadow: "10px 10px 0px rgba(0,0,0,0.5)" }}>
            OnikiKapı
            </h1>
...
// --- ÖNİZLEME PENCERESİ (MODAL) --- function SharePreviewModal({ dailyWisdom, onClose }) { const captureRef = useRef(null); const [downloading, setDownloading] = useState(false);

const handleDownload = async () => { if (captureRef.current && !downloading) { setDownloading(true); try { await document.fonts.ready; const dataUrl = await toPng(captureRef.current, { cacheBust: true, width: 1080, height: 1920, style: { transform: 'scale(1)', transformOrigin: 'top left', opacity: '1', visibility: 'visible', display: 'flex' } });
...
return ( <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/90 p-4 animate-fade-in backdrop-blur-md overflow-hidden"> <div className="relative w-full max-w-lg flex flex-col items-center gap-4"> <div className="flex justify-between items-center w-full text-white px-2"> <h3 className="text-lg font-bold text-gold">Önizleme</h3> <button onClick={onClose} className="p-2 bg-white/10 rounded-full hover:bg-white/20"><X size={24} /></button> </div>
...
// --- GÜNÜN MANEVİ GÖREVİ --- function GununGorevi({ task }) { return ( <div className="bg-gradient-to-br from-[#0F4C5C] to-[#09303a] p-10 rounded-3xl border border-gold/20 relative overflow-hidden shadow-xl mx-4 transition-all duration-500 hover:shadow-gold/10 group"> <div className="absolute -bottom-10 -left-10 p-4 opacity-5 rotate-12 pointer-events-none group-hover:opacity-10 transition-opacity"><HandHeart size={200} className="text-gold" /></div>
